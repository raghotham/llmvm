...
class ServerProxy:
    ...
    async def stream_chat(self, message: str) -> AsyncIterator[Chunk]:
        ...
        self._cancel_event.clear()  # Reset cancellation event
            ...
            async with httpx.AsyncClient(timeout=timeout_config) as client:
                ...
                async with request as response:
                    ...
                    async for line in response.aiter_lines():
                        if not self.is_streaming or self._cancel_event.is_set():  # Check if interrupted
                            break
    ...
    def interrupt(self):
        ...
        print("[DEBUG] Interrupt called - stopping stream")
        self.is_streaming = False
        self._cancel_event.set()  # Signal cancellation

        # Force close the HTTP connection if it exists
        if self.current_request:
            try:
                ...
                if loop.is_running():
                    ...
                    asyncio.create_task(self.current_request.aclose())
                else:
                    # If not in async context, run it synchronously
                    asyncio.run(self.current_request.aclose())
            except Exception as e:
                # Don't let close errors break the interrupt
