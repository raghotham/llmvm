...
from llmvm.common.helpers import Helpers, get_stream_handler, write_client_stream
    ...
    def __call__(self, *args, **kwargs):
        ...
        for name, param in inspect.signature(init_method).parameters.items():
            ...
            if param.annotation and param.annotation is Runtime:
                ...
                cookies = (
                    self.runtime.runtime_state["cookies"]
                    if "cookies" in self.runtime.runtime_state
                    else []
                )
...
class CallWrapper:
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(
            f"'{self.wrapped_class.__class__.__name__}' object has no attribute '{name}'"
        )
...
class Runtime:
    ...
    def setup(self) -> "Runtime":
        ...
        class float:
            def __new__(cls, value, *args, **kwargs):
                return builtins.float.__new__(
                    builtins.float, coerce_to(value, builtins.float), *args, **kwargs
                )
        ...
        class int(builtins.int):
            def __new__(cls, value, *args, **kwargs):
                return builtins.int.__new__(
                    builtins.int, coerce_to(value, builtins.int), *args, **kwargs
                )
    ...
    def create_todo(self, todo_description: str, expr_list: list[Any] = []) -> Todo:
        todo = Todo(
            id=len(self.runtime_state["todo_list"]),
            done=False,
    ...
    async def llmvm_call(
        self, context_or_content: list[Any], prompt: str
    ) -> list[Content]:
        ...
        messages = Helpers.flatten(
            [
                self.controller.statement_to_message(context_or_content),
        ...
        return Helpers.flatten(
            [
                MessageModel.to_message(message).message
                for message in session_thread.messages
            ]
        )
    ...
    async def delegate_task(
        ...
        """
        tasks_message = (
            [User(TextContent(user_tasks_message + "\n\n".join(user_tasks)))]
            if include_original_task
            else []
        )

        conversation, _ = await self.controller.aexecute_continuation(
            messages=tasks_message
            + self.controller.statement_to_message(expr_list)
            + [User(TextContent(task_description))],
            temperature=0.0,
        ...
        assistant = cast(
            Assistant,
            Helpers.last(
                lambda x: x.role() == "assistant",
                conversation,
                default=Assistant(TextContent("")),
            ),
        )
    ...
    def pandas_bind(self, expr) -> PandasMeta:
        ...
        def pandas_bind_with_llm(expr_str: str) -> PandasMeta:
            ...
            return PandasMeta(
                expr_str=expr_str, pandas_df=pd.DataFrame(assistant.get_str())
            )
        ...
        elif m_isinstance(expr, str) and (
            expr.startswith("gsheet://")
            or expr.startswith("https://docs.google.com/spreadsheets/")
        ):
            return Helpers.get_google_sheet(expr)
        elif m_isinstance(expr, str) and (".csv" in expr or expr.startswith("http")):
            try:
                result = urlparse(expr)

                if (
                    result.scheme == ""
                    or result.scheme == "file"
                    or result.scheme == "https"
                    or result.scheme == "http"
                ):
                    df = pd.read_csv(expr)
                    return PandasMeta(expr_str=expr, pandas_df=df)
                else:
                    logging.error(
                        f"PythonRuntime.pandas_bind({expr}) expr is an invalid URL"
                    )
    ...
    def count_tokens(self, content: list[Content] | Content | str) -> int:
        token_count = 0
        if isinstance(content, list) and Helpers.all(
            content, lambda x: isinstance(x, Content)
        ):
            token_count = asyncio.run(
                self.controller.get_executor().count_tokens([User(content)])
            )
            return token_count
            ...
            token_count = asyncio.run(
                self.controller.get_executor().count_tokens(
            ...
            token_count = asyncio.run(
                self.controller.get_executor().count_tokens(
                    [User([TextContent(Helpers.str_get_str(content))])]
                )
            )
            return token_count
    ...
    def last_assistant(self) -> list[Content]:
        ...
        result: Optional[Assistant] = Helpers.last(
            lambda x: x.role() == "assistant", self.messages_list
        )
        if result is None:
            return []
    ...
    def last_user(self) -> list[Content]:
        ...
        result: Optional[User] = Helpers.last(
            lambda x: x.role() == "user", self.messages_list
        )
        if result is None:
            return []
    ...
    def write_file(self, filename: str, content: list[Content] | str) -> bool:
        if os.path.basename(filename) != filename:
            logging.error(
                f"PythonRuntime.write_file() filename must be a basename, not a full path. filename: {filename}"
            )
            raise RuntimeError(
                f"write_file() filename must be a basename, not a full path. filename: {filename}"
            )

        memory_dir = Container().get_config_variable(
            "memory_directory",
            "LLMVM_MEMORY_DIRECTORY",
            default="~/.local/share/llmvm/memory",
        )
        ...
        def is_figure_or_image(c):
            return isinstance(c, ImageContent) or hasattr(c, 'savefig')

        is_binary = (filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp')) or
                    (hasattr(content, '__iter__') and not isinstance(content, str) and
                     any(is_figure_or_image(c) for c in (content if m_isinstance(content, list) else [content]))) or
                    is_figure_or_image(content))

        mode = "wb" if is_binary else "w"

        with open(
            os.path.expanduser(memory_dir) + f"/{self.thread_id}/{filename}", mode
        ) as f:

            def impl(c):
                if isinstance(c, ImageContent):
                    f.write(c.get_bytes())
                elif hasattr(c, 'savefig'):  # matplotlib Figure object
                    import io
                    buffer = io.BytesIO()
                    c.savefig(buffer, format='png', dpi=130, bbox_inches='tight')
                    f.write(buffer.getvalue())
                    buffer.close()
                else:
                    content_str = Helpers.str_get_str(c)
                    if mode == "wb":
    ...
    def read_file(self, full_path_filename: str) -> TextContent:
        ...
        if not os.path.exists(full_path_filename) and os.path.exists(
            f"{memory_dir}/{self.thread_id}/{full_path_filename_copy}"
        ):
            with open(f"{memory_dir}/{self.thread_id}/{full_path_filename}", "r") as f:
                return TextContent(
                    f.read(), url=f"{memory_dir}/{self.thread_id}/{full_path_filename}"
                )
    ...
    def write_memory(self, key: str, summary: str, value: list[Content] | str) -> bool:
        try:
            ...
            if not os.path.exists(
                os.path.expanduser(memory_dir) + f"/{self.thread_id}"
            ):
                os.makedirs(os.path.expanduser(memory_dir) + f"/{self.thread_id}")
            ...
            with open(
                os.path.expanduser(memory_dir) + f"/{self.thread_id}/{key}.meta", "w"
            ) as f:
                f.write(summary)

            with open(
                os.path.expanduser(memory_dir) + f"/{self.thread_id}/{key}.json", "w"
            ) as f:
                f.write(json.dumps(values, cls=ContentEncoder, indent=2))

            with open(
                os.path.expanduser(memory_dir) + f"/{self.thread_id}/{key}.txt", "w"
            ) as f:
    ...
    def read_memory(self, key: str) -> list[Content]:
        ...
        if not os.path.exists(
            os.path.expanduser(memory_dir) + f"/{self.thread_id}/{key}.json"
        ):
            return []

        with open(
            os.path.expanduser(memory_dir) + f"/{self.thread_id}/{key}.json", "r"
        ) as f:
    ...
    def read_memory_keys(self) -> list[dict[str, str]]:
        ...
        meta = [
            f
            for f in os.listdir(os.path.expanduser(memory_dir) + f"/{self.thread_id}")
            if f.endswith(".meta")
        ]
        key_summaries = [
            {
                "key": f.replace(".meta", ""),
                "summary": open(
                    os.path.expanduser(memory_dir) + f"/{self.thread_id}/{f}", "r"
                ).read(),
            }
            for f in meta
        ]
    ...
    def add_thread(
        ...
        if thread_id is None and program_name is None:
            raise ValueError(
                "add_thread() must be called with either thread_id or program_name"
            )
        ...
        if thread_id:
            ...
            thread: SessionThreadModel = asyncio.run(
                client.get_program(thread_id, program_name)
            )
            program_thread = "\n".join(
                [
                    MessageModel.to_message(message).get_str()
                    for message in thread.messages
                ]
            )
    ...
    def guard(
        ...
        raise ValueError(
            f"A guard() test was executed and it returned False or threw an exception. The error message is: {error_message}. The assertion code and values is:\n{dumped_assertion}"
        )
    ...
    def llm_var_bind(
        ...
        if (
            ...
            logging.debug(
                f"llm_var_bind({expr}) found in runtime_state, shortcircuiting"
            )
        ...
        lines = [linecache.getline(filename, call_lineno + i) for i in range(1, 10 + 1)]
        ...
        assistant = asyncio.run(
            self.controller.aexecute_llm_call(
                llm_call=LLMCall(
    ...
    def download(self, expr: list[str] | list[SearchResult]) -> list[Content]:
        ...
        cookies = (
            self.runtime_state["cookies"] if "cookies" in self.runtime_state else []
        )
        ...
        for url in expr:
            ...
            download_params.append(
                {"url": real_url, "goal": self.original_query, "search_term": ""}
            )

        if len(download_params) > 5:
            raise ValueError(
                "Too many downloads requested, max is 5, got {}".format(
                    len(download_params)
                )
            )

        result = asyncio.run(
            downloader.download_multiple_async(downloads=download_params)
        )
    ...
    def coerce(self, expr, type_name: Union[str, Type]) -> str:
        ...
        default_model = self.controller.get_executor().default_model
        logging.debug(
            f"coerce({str(expr)[:50]}, {str(type_name)}) length of expr: {len(str(expr))}"
        )
        ...
        logging.debug(
            "Coercing {} to {} resulted in {}".format(
                expr, type_name, assistant.get_str()
            )
        )
        write_client_stream(
            TextContent(
                f"Coercing {expr} to {type_name} resulted in {assistant.get_str()}. Trying to further coerce to {type_name}.\n"
            )
        )
    ...
    def llm_call(self, expr_list: list[Any] | Any, llm_instruction: str) -> Content:
        ...
        write_client_stream(
            TextContent(
                f'llm_call() calling {self.controller.get_executor().default_model} with instruction: "{llm_instruction}"\n'
            )
        )
    ...
    def llm_list_bind(
        ...
        list_type: Type[Any] = str,
    ) -> list[Any]:
        logging.debug(
            f"llm_list_bind({str(expr)[:20]}, {repr(llm_instruction)}, {count}, {list_type})"
        )
        ...
        try:
            ...
            logging.debug(
                f"PythonRuntime.llm_list_bind ex: {ex} list_result: {list_result}, llm_instruction {llm_instruction}"
            )
            raise RuntimeError(
                f"PythonRuntime.llm_list_bind error: {ex} list_result: {list_result}, llm_instruction {llm_instruction}",
                ex,
            )
    ...
    def helpers(self) -> str:
        helpers = "\n".join(
            [Helpers.get_function_description_flat(f) for f in self._helpers]
        )
        return helpers
    ...
    def locals(self) -> str:
        return "\n".join(
            [
                f"{key} = {Helpers.str_get_str(value)[:128]}"
...
def add_thread(
    thread_id: int, program_name: str, last_message: bool = False
) -> list[Content]:
    global _runtime
...
def guard(
    assertion: Callable[[], bool], error_message: str, bind_prompt: Optional[str] = None
) -> None:
    global _runtime
